...
def linemap_IFC(elements, height, ifc_name, slice_mode, fig_size='none'):
    """ Generate a linemap from an IFC file.
    ...
    for z in range(len(loaded_mesh)): #for every element, use transformation to get coordinates, generate polygons and plot the map
        ...
        for j in range(total_elements[z]):
            ...
            for polygon in polygons: #for each polygon, retrieve corner coordinates and plot them
...
def linemap_server(height, elements, slice_mode):
    """ Generate a linemap from a running GraphDB server
    ...
    for j in range(len(faces)): #rest from here is similar to Linemap_IFC above
